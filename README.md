...
A display inline is a property that reads as an inline element where the width and heights are not effected. Display inline has left and right margins but not top and bottom so it allows HTML beside it.
On the other hand, display block is displayed in block containers where it can have height and width values.
